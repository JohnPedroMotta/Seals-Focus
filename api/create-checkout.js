...
// Premium = assinatura recorrente (preapproval): cobra automático no cartão.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Premium = ASSINATURA RECORRENTE (cobra automático no cartão)
    if (item.premium) {
      const body = {
        reason: item.title,
        ...
        auto_recurring: {
          frequency: item.premium, // 1 = a cada 30 dias, 12 = a cada 12 meses
